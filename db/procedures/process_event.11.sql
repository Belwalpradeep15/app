    ...
    SET is_suppressed = EXISTS (
            ...
            AND (
                 (schedules.is_repeating != 't'
                  AND timezone('UTC', events.time) > (schedules.start_date || ' ' || schedules.start_time)::timestamp
                  AND timezone('UTC', events.time) <= (schedules.end_date || ' ' || schedules.end_time)::timestamp
                 )
                 OR
                 (schedules.is_repeating = 't'
                  AND timezone('UTC', events.time)> schedules.start_date
                  AND ( CASE WHEN schedules.start_time < schedules.end_time
                            THEN  (timezone('UTC', events.time))::time between schedules.start_time and schedules.end_time
                            ELSE  (timezone('UTC', events.time))::time NOT between schedules.end_time and schedules.start_time
                        END
                      )
                  AND ( CASE WHEN (schedules.start_time > schedules.end_time AND (timezone('UTC', events.time))::time between '00:00' and schedules.end_time  )
                               THEN schedules.repeats_on like '%' || extract(dow from timezone('UTC', events.time) - INTERVAL '1 day')  || '%'
                             ELSE
                                    schedules.repeats_on like '%' || extract(dow from timezone('UTC', events.time)) || '%'
                             END
                       )
                  AND (schedules.repeat_ends_on IS NULL OR (timezone('UTC', events.time) <= (schedules.repeat_ends_on || ' 23:59')::timestamp))
                 )
                )
             )
    WHERE e.id = event.id;
